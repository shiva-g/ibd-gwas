...
rule update_gen_sample:
    ...
    run:
        ...
        df_dat.loc[:, 'ID_2'] = df_dat.apply(lambda row: '0_' + row['iid'], axis=1)
        bad_samples = set(df_sample['ID_2']) - set(df_dat['ID_2'])
        bad_dat = []
        for bad_sample in bad_samples:
            bad_dat.append( [bad_sample, 'NA', 'NA',])
        bad_df = pd.DataFrame(bad_dat, columns=use_cols_short)
        df = pd.merge(df_sample, pd.concat([df_dat[['ID_2', 'sex', 'pheno']], bad_df]), how='left', on='ID_2')
        df[use_cols].to_csv(output.o, sep=' ', index=False)
        shell('cp {input.gen} {output.gen}')

rule gens:
    input:
        expand(DATA + 'interim/eur_gen/chr{c}.sample', c=range(22,23))
